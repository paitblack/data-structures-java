...
public class BinarySearchTree <T extends Comparable>{
    ...
    public boolean isCodeCorrect(String code){
        ...
        for(int i = 0; i<code.length();i++){
            ...
            if(current == '0' && iterator.left!= null ){
                iterator=iterator.left;
            }else if(current == '1' && iterator.right != null){
                iterator = iterator.right;
            }else{
                return false;
            }
        }
        return iterator.right == null && iterator.left == null;
    }
    public boolean search2(T val){
        TNode<T> iterator = root;
        while(iterator!= null){
            if(iterator.value.compareTo(val) == 0){
                return true;
